...
for(i in 1:nrow(data)){
  ...
  cur_nrf = RF.dist(pruned_tree, gt, normalize=T)
  # Calculate normalized RF between the pruned species tree and the gene tree
  
  cur_wrf = wRF.dist(pruned_tree, gt)
  # Calculate weighted RF between the pruned species tree and the gene tree
  
  cur_spr = SPR.dist(pruned_tree, gt)
  
  cur_kf = KF.dist(pruned_tree, gt)
  
  cur_path = path.dist(pruned_tree, gt)
  
  cur_wpath = path.dist(pruned_tree, gt, use.weight=T)
  ...
  data[i,]$spr = cur_spr
  data[i,]$kf = cur_kf
  data[i,]$path = cur_path
  data[i,]$wpath = cur_wpath
...
```{r nrf-dists, out.width="50%", fig.align = "center", warning=FALSE, fig.height=5}

x_comps = list(c("Accelerated", "Not accelerated"))

p1 = ggplot(data, aes(x=acc, y=nrf, group=acc, color=acc)) +
  #geom_boxplot() +
  geom_quasirandom(size=2, width=0.25, alpha=0.25) +
  geom_boxplot(outlier.shape=NA, alpha=0.75, width=0.5, color="#666666") +
  geom_signif(comparisons=x_comps, map_signif_level=TRUE, textsize=4, size=1, step_increase=0.12, margin_top=0.1, color="#333333") +
...
```{r wrf-dists, out.width="50%", fig.align = "center", warning=FALSE, fig.height=5}

x_comps = list(c("Accelerated", "Not accelerated"))

p1 = ggplot(subset(data, wrf < 8), aes(x=acc, y=wrf, group=acc, color=acc)) +
  #geom_boxplot() +
  geom_quasirandom(size=2, width=0.25, alpha=0.25) +
  geom_boxplot(outlier.shape=NA, alpha=0.75, width=0.5, color="#666666") +
  geom_signif(comparisons=x_comps, map_signif_level=TRUE, textsize=4, size=1, step_increase=0.12, margin_top=0.1, color="#333333") +
